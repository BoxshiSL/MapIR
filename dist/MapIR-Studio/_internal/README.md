...
[![Version](https://img.shields.io/badge/version-0.4.0-7c5cff.svg?style=flat-square)](pyproject.toml)
...
v0.4 adds an **optional local LLM drafting layer** (Ollama) — no cloud APIs,
no API keys, no model auto-downloads. Engine plugins are still on the roadmap.
...
This is the **Local LLM Drafting** release.

- 🧠 **Local LLM drafting layer** — draft WorldIR / SceneIR / DistrictProfile
  documents from a natural-language brief using a local model (Ollama by
  default). Output is validated by the same Pydantic + semantic rules as
  hand-authored IR — invalid drafts are never silently accepted.
- 🧩 **Provider abstraction** — `OllamaProvider` (real local model), `MockProvider`
  (deterministic fixtures for tests / Ollama-less workflows). Easy to extend
  with llama.cpp / generic HTTP in v0.5.
- 🗺 **Deterministic Plan → IR conversion** — the LLM produces a high-level
  Plan (no precise coordinates); MapIR deterministic helpers in
  `mapir/llm/plan_to_ir.py` compute validator-friendly geometry.
- 🛠 **Repair loop** — when validation fails, MapIR can hand the errors back to
  the model for a bounded number of repair attempts. Repaired output is
  re-parsed and re-validated before being accepted.
- 🖥 **LLM Draft page** in MapIR Studio — provider/model settings, task
  picker, brief input, raw / plan / IR / validation tabs, and explicit
  "Load Draft as Current Document" / "Save Draft As…" actions.
- ⌨ **CLI commands** — `llm-check`, `llm-draft-world`, `llm-draft-scene`,
  `llm-repair`. Use `--provider mock` for tests and demos that don't need a
  local model.
- 📚 **`docs/local_llm.md`** — Ollama setup, recommended models, structured
  JSON notes, troubleshooting.

No cloud APIs are introduced. No API keys are required. Existing IR
documents, CLI commands, examples, exporters, and CI gates are unchanged from
v0.3 — the LLM layer is purely additive.
...
MapIR v0.4 can draft WorldIR / SceneIR documents from a natural-language brief
using a **local** LLM. Default runtime: [Ollama](https://ollama.com). No cloud
APIs are used and no API keys are required.

You install and pull models yourself — MapIR never downloads them
automatically.
...
(Model availability depends on your local Ollama catalog — `ollama list`
shows what you actually have.)
...
:: Draft a world from a brief
python -m mapir.cli llm-draft-world ^
  --provider ollama --model qwen3:8b ^
  --brief "Create a compact crime city with port, old town, business core and offshore airport island." ^
  --out output\llm\world_draft.json

:: Draft a scene from a brief
python -m mapir.cli llm-draft-scene ^
  --provider ollama --model qwen3:8b ^
  --brief "Create a night urban alley with 3 entrances, 2 escape routes and 5+ cover markers." ^
  --out output\llm\scene_draft.json
```

For demos / tests that can't rely on a local model, swap `--provider ollama`
for `--provider mock` — MapIR ships a deterministic Mock provider that
returns validator-friendly fixtures.

### How it works

1. Build a strict-JSON prompt from your brief.
2. Call the local provider (Ollama by default).
3. Parse the model's JSON as a **Plan** (WorldPlan / ScenePlan / DistrictProfile).
4. Convert the Plan deterministically into WorldIR / SceneIR using
   `mapir/llm/plan_to_ir.py` (the LLM never invents coordinates).
5. Run the existing structural + semantic validators.
6. If invalid, ask the model to **repair** the output (bounded by
   `max_repair_attempts`); re-parse and re-validate.
7. Return the result to the CLI / desktop panel.

> **Local LLM output is always validated before use. Invalid drafts are not
> silently accepted.** See [`docs/local_llm.md`](docs/local_llm.md) for setup,
> recommended models, structured-JSON notes, and troubleshooting.

### Desktop UI

MapIR Studio gains an **LLM Draft** sidebar item. The page lets you pick the
provider/model/task, type a brief, kick off generation on a background thread
(so the UI doesn't freeze for 120s), and review four tabs of output — raw
text, plan JSON, IR JSON, and the validation report. Loading the draft into
the current document is an **explicit** click; MapIR never overwrites your
loaded document automatically.
...
`MapIR-Studio-v0.4.0-windows.zip` and attach it to a GitHub Release.
...
Honest scope check — none of the following are claimed by v0.4:

- ❌ Not a final art generator.
- ❌ Not a UE5 or Unity exporter.
- ❌ Not a procedural city / scene layout generator that produces shippable maps.
- ❌ Not a marketplace asset importer.
- ❌ No game-ready optimisation pipeline.
- ❌ No editable canvas (read-only inspector for now — LLM Draft writes a new
  document, but you cannot move/select objects in the preview).
- ❌ No installer yet (portable zip only).
- ❌ The local LLM layer is **assistive only** — the source of truth remains
  MapIR's schemas, validators, and your review.
...
**v0.3** — Desktop reset; PyInstaller exe; preflight; stable IR toolchain on
Windows.

**v0.4 (this release)** — Local LLM drafting layer: Ollama provider, Mock
provider, deterministic Plan → IR conversion, repair loop, LLM Draft desktop
page, four new CLI commands.

**v0.5** — Generic HTTP / llama.cpp provider, prompt template improvements
with few-shot examples, in-place draft editing, world ↔ scene embedding
workflow, asset-registry-assisted drafting.

**v0.6** — Editable canvas (move / select objects); UE5 exporter prototype;
road splines; data layers; gameplay markers in-engine; Blender preview
improvements.
